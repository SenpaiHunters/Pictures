window.navToggle = function () {
  ...
  const style = {
    ...
    nowplayingbarSelector:
      '.nav-ylx [data-testid="now-playing-bar"] :first-child:has([data-testid="player-controls"])',
  };
  ...
  function toggleSidebar() {
    ...
    leftSidebar.style.display = nowPlayingBar.classList.contains(
      style.leftSidebarCollapsedClassName
    )
      ? "none"
      : "block";
    mainView.style.width = nowPlayingBar.classList.contains(
      style.leftSidebarCollapsedClassName
    )
      ? "100%"
      : "";
  ...
  onLoad(() => {
    combinator.on(["control", "shift", "a"], toggleSidebar);
...
document.addEventListener("keydown", (e) => {
  ...
  if (
    ...
    navToggle.toggleSidebar(); // Call the toggleSidebar function when the hotkey is pressed
  }
});
